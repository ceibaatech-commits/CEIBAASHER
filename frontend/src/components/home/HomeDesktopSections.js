...
const ALL_KNOWN_CATEGORIES = EXAM_SECTIONS.map(s => s.key === 'language' ? null : null)
  .filter(Boolean);
...
/**
 * Configuration for all standard exam category sections.
 * Adding a new section = adding one entry to this array.
 */
const EXAM_SECTIONS = [
  {
    key: 'defence',
    ...
    filterFn: (exams) => exams.filter(e => ['NDA', 'Agniveer', 'CDS', 'CAPF'].includes(e.id)),
    hoverGlow: 'from-green-200 via-emerald-200 to-teal-200',
    hoverBorder: 'hover:border-green-300',
  },
  {
    ...
    borderColor: 'border-purple-200',
    iconBg: 'from-purple-600 via-blue-600 to-indigo-600',
    iconContent: <img src="https://customer-assets.emergentagent.com/job_prepninja-exams/artifacts/9pbxgmoq_Gemini_Generated_Image_1zgyxl1zgyxl1zgy_2-removebg-preview.png" alt="Admission" className="w-10 h-10 object-contain" />,
    title: 'Admission Tests',
    titleGradient: 'from-purple-700 via-blue-700 to-indigo-700',
    subtitle: '🎓 Gateway to Excellence • Professional Entrance Exams',
    tags: [
    ...
    filterFn: (exams) => exams.filter(e => e.category === 'Admission Tests'),
    hoverGlow: 'from-purple-600 via-blue-600 to-indigo-600',
    hoverBorder: 'hover:border-purple-300',
  },
  {
    key: 'banking',
  ...
  },
  {
    ...
    tags: [
      { text: '👮 Bihar Police', border: 'border-red-200' },
      { text: '🚔 Constable Recruitment', border: 'border-rose-200' },
      { text: '📋 BPSSC Exams', border: 'border-pink-200' },
    ],
    filterFn: (exams) => exams.filter(e => e.category === 'CSBC Examinations'),
    hoverGlow: 'from-red-200 via-rose-200 to-pink-200',
    hoverBorder: 'hover:border-red-300',
  },
  {
    ...
    tags: [
      { text: '📋 Patwari Exam', border: 'border-amber-200' },
      { text: '👮 Rajasthan Police', border: 'border-orange-200' },
      { text: '🎯 State Services', border: 'border-yellow-200' },
    ],
    filterFn: (exams) => exams.filter(e => e.category === 'RSMSSB Examinations'),
    hoverGlow: 'from-amber-200 via-orange-200 to-yellow-200',
    hoverBorder: 'hover:border-amber-300',
...
const EXCLUDED_IDS = ['NDA', 'Agniveer', 'CDS', 'CAPF'];
...
const HomeDesktopSections = ({ exams }) => {
  ...
  const otherExams = exams.filter(e =>
    !EXCLUDED_IDS.includes(e.id) && !EXCLUDED_CATEGORIES.includes(e.category)
  );
  ...
  return (
    <>
      ...
      {/* Language Proficiency Tests — special game-mode cards */}
      <div className="mb-16">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          className="bg-gradient-to-r from-purple-50 via-pink-50 to-rose-50 rounded-3xl p-8 mb-8 border-2 border-purple-200 shadow-lg"
        >
      ...
      {/* Other Competitive Exams — catch-all for uncategorised exams */}
      {otherExams.length > 0 && (
        <HomeExamSection
          bgGradient="from-gray-50 via-slate-50 to-gray-50"
          borderColor="border-gray-200"
          ...
          title="Other Competitive Exams"
          titleGradient="from-gray-700 via-slate-700 to-zinc-700"
          subtitle="📖 More Opportunities • Agriculture & State Services"
          tags={[
            { text: '🌾 Agriculture Exams', border: 'border-gray-200' },
            { text: '📋 RPSC & State Exams', border: 'border-slate-200' },
            { text: '🎯 Specialized Services', border: 'border-zinc-200' },
          ]}
          exams={otherExams}
          hoverGlow="from-gray-200 via-slate-200 to-zinc-200"
          hoverBorder="hover:border-gray-300"
