...
const BattleVideoChat = ({ socket, roomId, playerName, opponentName, opponentId }) => {
  ...
  // Sanitize channel name (same logic used everywhere)
  const sanitizedChannel = roomId?.replace(/[^a-zA-Z0-9]/g, '').substring(0, 64) || '';

  // Fetch Agora token when roomId changes
  useEffect(() => {
    ...
    const fetchToken = async () => {
      try {
        ...
        // Fallback to empty string (no token auth) if server has no certificate
        setAgoraToken('');
      }
    };
    fetchToken();
  ...
  const callbacks = {
    EndCall: () => console.log('[Agora] Call ended'),
  };
          ...
          <div className="bg-white rounded-2xl max-w-md w-full max-h-[85vh] overflow-y-auto shadow-2xl">
            {reportDone ? (
              <div className="p-8 text-center">
                ...
                <p className="text-gray-600 text-sm mb-3">Report received against <strong>{opponentName}</strong>.</p>
