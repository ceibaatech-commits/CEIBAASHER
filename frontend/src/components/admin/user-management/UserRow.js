...
import { Calendar, Shield, Eye, Mail, Ban } from 'lucide-react';
...
export const UserRow = ({ user, onToggleBadge }) => (
  <tr className="hover:bg-gray-50 transition-colors" data-testid={`user-row-${user.id}`}>
    ...
    <td className="px-6 py-4 whitespace-nowrap"><StatusPill status={user.status} /></td>
    <td className="px-6 py-4 whitespace-nowrap">
    ...
    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
      <div className="flex items-center space-x-2">
        ...
        <button className="p-2 text-gray-600 hover:bg-gray-50 rounded-lg transition-colors" title="Send Email"
          data-testid={`email-user-${user.id}`}>
          <Mail className="w-4 h-4" />
        </button>
        <button className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Ban User"
