...
export default function CommentModal({ postId, onClose }) {
  ...
  const handleLikeComment = async (commentId) => {
    ...
    try {
      ...
      setComments(prev => updateLikeHelper(prev, commentId, data));
    } catch {}
  };
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {loading ? <div className="text-center text-slate-400 py-8">Loading...</div> : comments.length === 0 ? (
            ...
            <div key={c.id} className="space-y-2">
              <CommentItem c={c} isReply={false} handleLikeComment={handleLikeComment} setReplyTo={setReplyTo} />
              {(c.replies || []).map(r => <CommentItem key={r.id} c={r} isReply={true} handleLikeComment={handleLikeComment} setReplyTo={setReplyTo} />)}
            </div>
          ))}
        </div>
        {replyTo && (
...
const updateLikeHelper = (list, commentId, data) => 
  list.map(c => {
    if (c.id === commentId) return { ...c, likes_count: data.likes_count, is_liked: data.liked };
    if (c.replies?.length) return { ...c, replies: updateLikeHelper(c.replies, commentId, data) };
    return c;
  });

const CommentItem = ({ c, isReply, handleLikeComment, setReplyTo }) => (
